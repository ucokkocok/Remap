...
{
	if (argc < 4) {
		std::cout << skCrypt("Usage: FakeProc GameProc Remap/Restore") << std::endl;
		system("pause");
		return 0;
	}
	...
	Isret = Common.OpenDriver(skCrypt(L"\\\\.\\PEAUTH"));
	if (!Isret)
	{ 
		std::cout << skCrypt("[-] Failed to Open Driver") << std::endl;
		system("pause");
		return 0;
	}
	...
	{
		std::cout << skCrypt("[+] Loading Driver... ") << std::endl;
		...
		if (!kdmapper::MapDriver(iqvw64e_device_handle, rawData, sizeof(rawData), 0, 0, false, true, true, true, 0, &exitCode)) {
			std::cout << skCrypt("[-] Failed to LoadDriver") << std::endl;
			intel_driver::Unload(iqvw64e_device_handle);
			system("pause");
			return 0;
		}
	...
	bool Remap = GetParam(argc, argv, skCrypt("Remap")) > 0;
	bool Restore = GetParam(argc, argv, skCrypt("Restore")) > 0;
	...
	if (Remap)
	{
		...
		std::cout << skCrypt("[+] Process was Remaped") << std::endl;
	}
	else if (Restore)
	{
		...
		std::cout << skCrypt("[+] Process was Restored") << std::endl;
	}
	else
	{
		std::cout << skCrypt("[-] Wrong param") << std::endl;
	}
